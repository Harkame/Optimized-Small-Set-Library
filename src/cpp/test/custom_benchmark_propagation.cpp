...
void test_propagate_micro_inner_set()
{
 ...
 void test_propagate_micro_vector_set()
 {
   ...
   long t_size = 0;
   inner_vector_set<int>* t_inner_vector;

   for(int t_index = 0; t_index < g_list_micro_vector_set.size(); t_index++)
   {
     t_inner_vector = reinterpret_cast<inner_vector_set<int>* >(g_list_micro_vector_set.at(t_index)->a_inner_set);
     t_size += t_inner_vector->capacity() * sizeof(int);
  }
