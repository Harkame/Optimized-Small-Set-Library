...
inner_set<T, p_to_up>* inner_set_3<T, p_to_up>::add_element(T p_element)
{
	if(!contains_element(p_element))
		if(this->a_to_up == 3)
			...
			return new inner_set_4<T, p_to_up>(this, p_element);
	else
		return this;
}
...
{
   return  p_inner_set->contains_element(a_values.element_1)  &&   p_inner_set->contains_element(a_values.element_2)  &&   p_inner_set->contains_element(a_values.element_3) ;
}
...
inner_set<T, p_to_up>* inner_set_3<T, p_to_up>::remove_element(T p_element)
{
                ...
                {
        	delete this;
        	return new inner_set_2<T, p_to_up>( a_values.element_2,  a_values.element_3);
    	}
         else                 if(a_values.element_2 == p_element)
                {
        	delete this;
        	return new inner_set_2<T, p_to_up>( a_values.element_1,  a_values.element_3);
    	}
         else                 if(a_values.element_3 == p_element)
                {
        	delete this;
        	return new inner_set_2<T, p_to_up>( a_values.element_1,  a_values.element_2);
    	}
                        else
            return this;
}
...
{
	if( a_values.element_1 == p_element  ||   a_values.element_2 == p_element  ||   a_values.element_3 == p_element )
		return this;
	else
  	{
