...
class InnerSet_2[T](element1: T, element2: T) extends InnerSet[T]
{
    ...
    override def addElement(p_element: T): InnerSet[T] =
    {
        ...
        else  new InnerSet_3(element1element2, p_element)
    }

    override def addUnchecked(p_element: T): InnerSet[T] = InnerSet_3(element1, p_element)element2, p_element)

    override def addAllElements(p_innerSet: InnerSet[T]): InnerSet[T] =     {
                p_innerSet.addElement(element1)        p_innerSet.addElement(element2)    }
    ...
    override def removeElement(p_element: T): InnerSet[T] =
    {
            if(element1.equals(p_element)){
             new InnerSet_1(element2)                     if(element2.equals(p_element)){
    ...
    override def removeAllElements(p_innerSet: InnerSet[T]): InnerSet[T] =     {
                p_innerSet.removeElement(element1)        p_innerSet.removeElement(element2)    }

    override def retainAllElements(p_innerSet: InnerSet[T]): InnerSet[T] =
    {
      var i: InnerSet[T] = this
            if(!p_innerSet.containsElements(element1)) i = i.removeElement(element1)      if(!p_innerSet.containsElements(element2)) i = i.removeElement(element2)      i
    }
