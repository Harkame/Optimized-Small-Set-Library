...
class MicroSet_Benchmark_Remove {
  ...
  def setUp() = {
    ...
    for (i <- 0 to runner.NUMBER_OF_TEST_OBJECT) tabTestObject(i) = TestObject(rand.nextInt())
    for (i <- 0 to runner.NUMBER_OF_TEST_OBJECT / 2) {
  ...
  def testRemove_TreeSet(reps: Int) = {
    for (i <- 0 to reps) {
      for (j <- 0 until runner.NUMBER_OF_TEST_OBJECT / 4) treeSet.remove(tabTestObject(tabRandomInt(j)))
    }
  }
  ...
  def testRemove_HashSet(reps: Int) = {
    for (i <- 0 to reps) {
      for (j <- 0 until runner.NUMBER_OF_TEST_OBJECT / 4) hashSet.remove(tabTestObject(tabRandomInt(j)))
    }
  }
  ...
  def testRemove_InnerSet(reps: Int) = {
    for (i <- 0 to reps) {
      for (j <- 0 until runner.NUMBER_OF_TEST_OBJECT) microInnerSet.remove(tabTestObject(tabRandomInt(j)))
    }
  }
  ...
  def testRemove_InnerArraySet(reps: Int) = {
    for (i <- 0 to reps) {
      for (j <- 0 until runner.NUMBER_OF_TEST_OBJECT) microArraySet.remove(tabTestObject(tabRandomInt(j)))
    }
  }
  ...
  def testRemove_InnerHashSet(reps: Int) = {
    for (i <- 0 to reps) {
      for (j <- 0 until runner.NUMBER_OF_TEST_OBJECT) microHAshSet.remove(tabTestObject(tabRandomInt(j)))
    }
  }
}
